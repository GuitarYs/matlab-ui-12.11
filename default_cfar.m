...
% PARAM: D, double, 0   
    ...
    D = getParam(params, 'D', 0);  % 新增：从params里拿D
    ...
    % CFAR处理
    window_size = 2 * (guard_cells + training_cells) + 1; %#ok<NASGU>
    % window_size 当前没直接用到，但先留着，保持原逻辑
    ...
    output_data.threshold_factor = threshold_factor;     % 阈值因子
    output_data.D = D;
    ...
    try
        ...
        axis(ax, 'on');
        title(ax, sprintf('CFAR检测结果 - 方法:%s, 阈值因子:%.1f, D:%.3f', method, threshold_factor, D));
        xlabel(ax, '距离单元啥时');
        ylabel(ax, '多普勒单元收拾收拾');
